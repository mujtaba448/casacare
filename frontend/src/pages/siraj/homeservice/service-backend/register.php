...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($phone) && !preg_match('/^\d{10,15}$/', $phone)) {
        http_response_code(400);
        echo json_encode(['error' => 'Phone number must be between 10-15 digits and contain only numbers.']);
        exit;
    }
    ...
    try {
        ...
        // Log the actual error for debugging (in production, don't expose this to users)
